...
describe("openai transport stream", () => {
  ...
  //
  // The pre-stream `createResponsesStreamWithEncryptedContentRetry` only covers
  // failures thrown by `client.responses.create()`. In v2026.6.8 the poison
  // arrives DURING streaming as a `response.failed` SSE event thrown inside
  // `processResponsesStream`, which lands in the outer catch (no retry). These
  // gates lock the fail-fast drain-level retry: reuse the SAME already-started
