 ...
 *
 * `createResponsesStreamWithEncryptedContentRetry` above only covers failures
 * thrown by `client.responses.create()` (before streaming starts). The poison
 * encrypted reasoning, however, is usually rejected mid-stream as a
 * `response.failed` SSE event, which `processResponsesStream` throws. That throw
 * previously bubbled to the outer transport catch with no retry.
 *
 * This helper wraps create + drain so a streamed encrypted-content failure can
 * be retried ONCE with a sanitized request. It is fail-fast and safe: it only
 * retries when NO assistant content has been committed yet (the single `start`
 * event is owned by the caller and is emitted exactly once), so the user never
 * sees a duplicate `start`, duplicate partial, or duplicate `done`. The reused
 * `output` is mutated in place; on a clean retry the successful response id
 * overwrites the failed one.
 */
